...
using Version1 = CriticalCompendiumInfrastructure.Persistence.Version1;
using Version2 = CriticalCompendiumInfrastructure.Persistence.Version2;
...
namespace CritCompendium
{
   public sealed class DataManager : IDataManager
   {
      ...
      public DataManager(XMLImporter xmlImporter, XMLExporter xmlExporter)
      {
         ...
         _saveDataFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Critical Compendium");
      ...
      public void SaveCharacters(IEnumerable<CharacterModel> characters)
      {
         ...
         {
            Steamworks.SteamRemoteStorage.FileWrite(_characterSaveFileName, characterBytes);
         }
      }
      ...
      public void SaveEncounters(IEnumerable<EncounterModel> encounters)
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_encountersSaveFileName, encounterBytes);
         }
      }
      ...
      public void SaveBackgrounds(IEnumerable<BackgroundModel> backgrounds)
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_backgroundsSaveFileName, Encoding.UTF8.GetBytes(xml));
         }
      }
      ...
      public void SaveClasses(IEnumerable<ClassModel> classes)
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_classesSaveFileName, Encoding.UTF8.GetBytes(xml));
         }
      }
      ...
      public void SaveConditions(IEnumerable<ConditionModel> conditions)
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_conditionsSaveFileName, Encoding.UTF8.GetBytes(xml));
         }
      }
      ...
      public void SaveFeats(IEnumerable<FeatModel> feats)
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_featsSaveFileName, Encoding.UTF8.GetBytes(xml));
         }
      }
      ...
      public void SaveItems(IEnumerable<ItemModel> items)
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_itemsSaveFileName, Encoding.UTF8.GetBytes(xml));
         }
      }
      ...
      public void SaveLanguages(IEnumerable<LanguageModel> languages)
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_languagesSaveFileName, Encoding.UTF8.GetBytes(languagesString));
         }
      }
      ...
      public void SaveMonsters(IEnumerable<MonsterModel> monsters)
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_monstersSaveFileName, Encoding.UTF8.GetBytes(xml));
         }
      }
      ...
      public void SaveRaces(IEnumerable<RaceModel> races)
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_racesSaveFileName, Encoding.UTF8.GetBytes(xml));
         }
      }
      ...
      public void SaveSpells(IEnumerable<SpellModel> spells)
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_spellsSaveFileName, Encoding.UTF8.GetBytes(xml));
         }
      }
      ...
      public void SaveLaunchData()
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            Steamworks.SteamRemoteStorage.FileWrite(_dataSaveFileName, bytes.ToArray());
         }
      }
      ...
      private void CheckSteamCloud()
      {
         ...
         if (Steamworks.SteamClient.IsValid)
         {
            string path = Path.Combine(_saveDataFolder, _dataSaveFileName);

            if (!File.Exists(path))
            {
               foreach (string remoteFileName in Steamworks.SteamRemoteStorage.Files)
               {
                  byte[] remoteBytes = Steamworks.SteamRemoteStorage.FileRead(remoteFileName);
                  File.WriteAllBytes(Path.Combine(_saveDataFolder, remoteFileName), remoteBytes);
               }
            }
            else
            {
               DateTime localDate = DateTime.MinValue;
               DateTime remoteDate = DateTime.MinValue;

               byte[] localDataFile = File.ReadAllBytes(path);
               if (localDataFile != null && localDataFile.Length > 11)
               {
                  long date = BitConverter.ToInt64(localDataFile.Skip(4).Take(8).ToArray(), 0);
                  localDate = new DateTime(date);
               }

               byte[] remoteDataFile = Steamworks.SteamRemoteStorage.FileRead(_dataSaveFileName);
               if (remoteDataFile != null && remoteDataFile.Length > 11)
               {
                  long date = BitConverter.ToInt64(remoteDataFile.Skip(4).Take(8).ToArray(), 0);
                  remoteDate = new DateTime(date);
               }

               if (localDate != DateTime.MinValue && remoteDate != DateTime.MinValue && localDate < remoteDate)
               {
                  foreach (string remoteFileName in Steamworks.SteamRemoteStorage.Files)
                  {
                     byte[] remoteBytes = Steamworks.SteamRemoteStorage.FileRead(remoteFileName);
